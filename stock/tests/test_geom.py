...
from stock.geom import radius_at as _radius_at_core, append_post_segment_from_row  # existing helpers
...
def _build_tine_from_row(row_dict: dict, radius_at_callable) -> tuple[Part.Shape, str]:
    """
    Extracted (no-behavior-change) tine builder.
    Returns (shape, summary_str) and prints the same logs as before.
    Supports both 'wedge' (legacy) and 'plate' (current) types.
    """
    ...
    # Extend length so the lower edge reaches far enough after rotation
...
def build_stock_from_csv(doc: App.Document) -> App.DocumentObject:
    ...
    for row_dict in rows:
        ...
        if not shape_type and 'wedge' in row_dict:
            shape_type = 'wedge'  # legacy key for tine section (unchanged)
        ...
        try:
            if shape_type == 'cylinder':
                ...
                wedge, summary = _build_tine_from_row(row_dict, _radius_at)
...
def create_drawing_page(doc: App.Document, stock_obj: App.DocumentObject,
    ...
    side_view.X = 90
    side_view.Y = 128
    side_view.SeamVisible = True      # keep cylinder/trim seams
    side_view.SmoothVisible = True    # optional; shows tangent edges
